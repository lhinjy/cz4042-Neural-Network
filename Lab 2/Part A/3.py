...
def main():
    ...
    learning_rate = 0.001
    optimizer_ = ['SGD','SGD-momentum','RMSProp','Adam']  # Question 3
    ...
    for i in optimizer_:
        ...
        elif i == 'SGD-momentum':  # Question 3(a)
            optimizer = keras.optimizers.SGD(learning_rate=learning_rate, momentum=momentum)
        elif i == 'RMSProp':  # Question 3(b)
            optimizer = tf.keras.optimizers.RMSprop(learning_rate=learning_rate)
        elif i == 'Adam':  # Question 3(c)
        ...
        # Save model
        if use_dropout:
            model.save(f'./{save_path}/models/{num_ch_c1}_{num_ch_c2}_{i}_dropout.h5')
        else:
            model.save(f'./{save_path}/models/{num_ch_c1}_{num_ch_c2}_{i}_no_dropout.h5')
        ...
        plt.xlabel('epoch')
        plt.legend(loc='best')
        plt.savefig(f'./{save_path}/results/{num_ch_c1}_{num_ch_c2}_{i}_no_dropout_loss.png')
        plt.close()
        plt.show()
        ...
        plt.xlabel('epoch')
        plt.legend(loc = 'best')
        plt.savefig(f'./{save_path}/results/{num_ch_c1}_{num_ch_c2}_{i}_no_dropout_accuracy.png')
        plt.close()
        plt.show()
