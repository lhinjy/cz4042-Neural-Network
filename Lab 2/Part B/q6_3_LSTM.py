...
class CharRNN(Model):
    ...
    def __init__(self, vocab_size, hidden_dim=10):
        ...
        self.lstm = tf.keras.layers.LSTM(self.hidden_dim, unroll=True)
    ...
    def call(self, x, drop_rate):
        ...
        # lstm = self.rnn(embedding)
        lstm = self.lstm(embedding)
        lstm = tf.nn.dropout(lstm, drop_rate)
...
pylab.savefig('./%s/3_LSTM_accuracy.png'%(save_path))
pylab.close()
...
pylab.legend(loc='best')
pylab.savefig('./%s/3_LSTM_loss.png'%(save_path))
pylab.close()
