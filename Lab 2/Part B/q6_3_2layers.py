...
class CharRNN(Model):
    ...
    def __init__(self, vocab_size, hidden_dim=10):
        ...
        # rnn_cells = [layers.GRU(self.hidden_dim, unroll=True) for _ in range(2)]
        # self.stacked_gru = tf.nn.rnn_cell.MultiRNNCell(rnn_cells)
    ...
    def call(self, x, drop_rate):
        ...
        encoding = rnnl1(embedding)
        encoding = rnnl2(embedding)
...
pylab.savefig('./%s/3_2layer_accuracy.png'%(save_path))
pylab.close()
...
pylab.legend(loc='best')
pylab.savefig('./%s/3_2layer_loss.png'%(save_path))
pylab.close()
